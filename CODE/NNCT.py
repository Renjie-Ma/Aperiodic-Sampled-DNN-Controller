'''
This file <NNCT.py>
- utilizes the generated state-input data pairs <exp_data2204.csv> to train the feedforward controller \pi_0;
- extracts the trained deterministic parameters-->>'W_1', 'W_2', and 'W_3'
Authored by R. Ma @ Harbin Institute of Technology
Updated April 2022
- The DNN controller \pi_0 involves 2 hidden layers with neurons 32 and 16, respectively;
- The activation function is selected by 'tanh' with no bias terms
- 'Adam' optimizer
'''
# import modules 
...
class FeatureDataset(Dataset):  # troch.utils.data --> generate torch tensors and have torch compatible dataset
    def __init__(self,file_name):
        ...
        '''
        file_out = pd.read_csv(file_name) # The file <exp_data3.csv> has 8798 rows, each of which has 5 columes
        xx = file_out.iloc[:,0:2].values  # Here, we have read our file  
        uu = file_out.iloc[:, 2].values   
        # Notice: the index problem is explained by https://dataanalyticsireland.ie/2021/04/04/indexerror-single-positional-indexer-is-out-of-bounds/
        # 可以预处理一下啊
        # feature scaling
        '''
...
# Define FFNN model
# The input (4); the first hidden layer (32); the second hidden layer; the output (1)
# The neural network with no-bias prescription
class FFNNController(torch.nn.Module):
    def __init__(self):
        super(FFNNController,self).__init__()
        self.flatten = torch.nn.Flatten() # https://pytorch.org/docs/stable/generated/torch.flatten.html
        ...
        ) # The input (4); The first hidden layer (32); The second hidden layer (16); The output (1)
...
feature_data = FeatureDataset('F:\\Papers\\AperiodicSampledDNNController\\Codes\\NN_Policy\\exp_data3.csv')  # Load the stored data file
...
def main():
    ...
    torch.save(model.state_dict(),'F:\\Papers\\AperiodicSampledDNNController\\Codes\\save_weight0.pt') # Save the trained NN weights
